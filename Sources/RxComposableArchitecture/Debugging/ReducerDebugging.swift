...
extension Reducer {
    ...
    public func debug(
        ...
        debug(prefix, state: { $0 }, action: .self, environment: toDebugEnvironment)
    }
    ...
    public func debugActions(
        ...
        debug(prefix, state: { _ in () }, action: .self, environment: toDebugEnvironment)
    }
    ...
    public func debug<LocalState, LocalAction>(
        ...
        environment toDebugEnvironment: @escaping (Environment) -> DebugEnvironment = { _ in
            ...
            return .init { state, action, environment in
                ...
                let effects = self.run(&state, action, environment) { state in
                    guard let localAction = toLocalAction.extract(from: action) else { return }
